...
Imagesize=[64,64];
pretotal=49;
trainpath='D:\li\PIE\train\';
testpath='D:\li\PIE\test\';
...
for j=1:num_people
    for i=1:pretrain
        im=imread([trainpath,int2str(j),'\',int2str(i),'.bmp']);
        iim = rgb2gray(im);
...
for j=1:num_people
    for i=1:pretotal-pretrain
        im=imread([trainpath,int2str(j),'\',int2str(i+pretrain),'.bmp']);
        iim = rgb2gray(im);
...
fprintf('\n     Angle Testing recognition rate: %.2f%%', 100-100*ErRate);
