...
%dup-1
labeltrain=2;      %dup-2时为4，fb时为1，fc时为1。
labeltest=3;        %dup-2时为5，fb时为6，fc时为7.
num_people=20;
pretrain=1;
Imagesize=[80,80];
pretest=1;
trainpath='C:\Users\11966\Desktop\pca\L1Test\train\';
testpath='C:\Users\11966\Desktop\pca\L1Test\test\';
TrnData=zeros(Imagesize(1)*Imagesize(2),num_people*pretrain);
TrnLabels=zeros(num_people*pretrain,1);
TestData=zeros(Imagesize(1)*Imagesize(2),num_people*pretest);
TestLabels=zeros(num_people*pretest,1);
for j=1:num_people
    for i=1:pretrain
        iim=imread([trainpath,int2str(j),'\',int2str(labeltrain),'.png']);
%         iim = rgb2gray(im);
...
for j=1:num_people
    for i=1:pretest
        iim=imread([trainpath,int2str(j),'\',int2str(labeltest),'.png']);
%         iim = rgb2gray(im);
        TestData(:,i+(j-1)*pretest)=reshape(iim,Imagesize(1)*Imagesize(2),1);
        TestLabels(i+(j-1)*pretest,:)=j;
    end
end
